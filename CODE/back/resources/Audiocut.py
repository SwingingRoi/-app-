...
def cut(name,directory):
    ...
    joint_silence_len = 0  # 段拼接时加入1300毫秒间隔用于断句

    # 将录音文件拆分成适合百度语音识别的大小
    path=prepare_for_baiduaip(name,directory,sound, silence_thresh, min_silence_len, length_limit, abandon_chunk_len,
                                 joint_silence_len)
    for i in path:
        print(i)


def prepare_for_baiduaip(name,directory,sound, silence_thresh=-70, min_silence_len=700, length_limit=60 * 1000,
    ...
    for i in range(total):
        newpath=pathfold+str(i)+".mp3"
        path.append(newpath)
        chunks[i].export(newpath, format="mp3")
...
def chunk_split_length_limit(chunk, min_silence_len=700, length_limit=60 * 1000, silence_thresh=-70, level=0):
    ...
    if len(chunk) > length_limit:
        ...
        if min_silence_len <= 0:
            tempname = 'C:/Users/09/Desktop/chunks/temp_%d.mp3' % int(time.time())
